...
$project=$dc_id=$block=$day=$month=$year="";
...
echo '<div class="heading">SUMMARY BY BC</div>';
...
$query =    "SELECT * ,count(*) as total
    FROM stateco join distco ON stateco.sc_id = distco.sc_id 
                 ...
                 $where 
                  GROUP BY blockco.block order by distco.district,blockco.block,areaco.nps_id,benf.benf_date";
...
if($blk<>$block){
echo '<td class="benf"><a href="sub_sum_by_nps.php?benf_date='.$benf_date.'&project='.$project.'&dc_id='.$dc_id.'&block='.$block.'&day='.$day.'&month='.$month.'&year='.$year.'">'.$block.' </a></td>';
$blk=$block;
}else{
    echo '<td class="benf"></td>';
}   
...
if($dist<>$district and $bc<>$bc_name ){
  echo '<td class="benf">'.'S/TOTAL '.$proj_nos.'NOS'.'</td>';
  }
echo '</tr>';
...
echo '<tr>';
echo '<td class="benf" colspan="5">TOTAL</td>';
